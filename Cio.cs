...
public class Cio // Character In/Out
{
    ...
    /// </summary>
    public bool CanSeek => _s.CanSeek;
...
internal class BufferCio
{
    ...
    public void SetBuffer(ref byte[] buffer, uint neededSize)
    {
        if (neededSize > buffer.Length)
            Array.Resize(ref buffer, (int)neededSize);
        _buffer = buffer;
    }

    public void SetBuffer(uint neededSize)
    {
        if (neededSize > _buffer.Length)
            Array.Resize(ref _buffer, (int)neededSize);
    }
}