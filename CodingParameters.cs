...
public class CompressionParameters
{
    ...
    /// </summary>
    public bool Irreversible { get => irreversible;
        set => irreversible = value;
    }
    ...
    {
        get => new(subsampling_dx, subsampling_dy);
        set
        {
            if (value == null)
            {
    ...
    {
        get => new(image_offset_x0, image_offset_y0);
        set
        {
            if (value == null)
            {
    ...
    {
        get => new(cblockw_init, cblockh_init);
        set
        {
            if (value == null)
            {
    ...
    /// </summary>
    public bool WriteSOP { get => (csty & CP_CSTY.SOP) != 0;
        set { if (value) csty |= CP_CSTY.SOP; else csty &= ~CP_CSTY.SOP; } }

    /// <summary>
    /// Writes a EPH (End packet header) marker after each header packet
    /// </summary>
    public bool WriteEPH { get => (csty & CP_CSTY.EPH) != 0;
        set { if (value) csty |= CP_CSTY.EPH; else csty &= ~CP_CSTY.EPH; } }
    ...
    public bool IsBroadcast => rsiz >= J2K_PROFILE.BC_SINGLE && (int)rsiz <= ((int)J2K_PROFILE.BC_MULTI_R | 0x000b);

    public bool IsIMF => rsiz >= J2K_PROFILE.IMF_2K && (int)rsiz <= ((int)J2K_PROFILE.IMF_8K_R | 0x009b);

    public bool IsPart2 => (rsiz & J2K_PROFILE.PART2) == J2K_PROFILE.PART2;
...
internal class CodingParameters
{
    ...
    {
        get => ppm_markers != null ? (uint)ppm_markers.Length : 0u;
        set
        {
